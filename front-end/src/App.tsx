...
const INITIAL_PROCESSES: Process[] = [
  { id: 'p1', name: 'P1', arrivalTime: 0, burstTime: 5, priority: 3, color: PRESET_COLORS[0] },
  { id: 'p2', name: 'P2', arrivalTime: 2, burstTime: 3, priority: 1, color: PRESET_COLORS[1] },
  { id: 'p3', name: 'P3', arrivalTime: 4, burstTime: 2, priority: 4, color: PRESET_COLORS[2] },
  { id: 'p4', name: 'P4', arrivalTime: 6, burstTime: 4, priority: 2, color: PRESET_COLORS[3] },
];
...
export default function App() {
  ...
  const [apiUrl, setApiUrl] = useState<string>('http://localhost:8080/simulate');
  ...
  useEffect(() => {
    ...
    const loadSimulation = async () => {
      ...
      try {
        ...
        if (active) {
          setError(err.message || 'Erro ao conectar à API do simulador.');
          setIsPlaying(false);
        }
      } finally {
        if (active) {
  ...
  const processStats = simulationResult?.processStats || {};
  ...
  // Sync current time bounds
  useEffect(() => {
    if (timeline.length > 0 && currentTime >= timeline.length) {
      setCurrentTime(Math.max(0, timeline.length - 1));
      setIsPlaying(false);
    }
  ...
  const handleRemoveProcess = (id: string) => {
    ...
    const updated = processes.filter((p) => p.id !== id).map((p, idx) => ({
      ...p,
      name: `P${idx + 1}`,
    }));
  ...
  const handleLoadClassicPreset = () => {
    setProcesses([
      { id: 'p1', name: 'P1', arrivalTime: 0, burstTime: 6, priority: 3, color: PRESET_COLORS[0] },
      { id: 'p2', name: 'P2', arrivalTime: 2, burstTime: 3, priority: 1, color: PRESET_COLORS[1] },
      { id: 'p3', name: 'P3', arrivalTime: 4, burstTime: 1, priority: 4, color: PRESET_COLORS[2] },
      { id: 'p4', name: 'P4', arrivalTime: 5, burstTime: 4, priority: 2, color: PRESET_COLORS[3] },
    ]);
    setAlgorithm('RR');
    setQuantum(2);
    setOverloadTime(1);
  ...
  const handleLoadConvoyPreset = () => {
    ...
    setProcesses([
      { id: 'p1', name: 'P1', arrivalTime: 0, burstTime: 12, priority: 3, color: PRESET_COLORS[0] },
      { id: 'p2', name: 'P2', arrivalTime: 1, burstTime: 2, priority: 2, color: PRESET_COLORS[1] },
      { id: 'p3', name: 'P3', arrivalTime: 1, burstTime: 2, priority: 1, color: PRESET_COLORS[2] },
    ]);
    setAlgorithm('FIFO');
  ...
  const handleLoadPriorityPreset = () => {
    setProcesses([
      { id: 'p1', name: 'P1', arrivalTime: 0, burstTime: 5, priority: 4, color: PRESET_COLORS[0] },
      { id: 'p2', name: 'P2', arrivalTime: 1, burstTime: 4, priority: 2, color: PRESET_COLORS[1] },
      { id: 'p3', name: 'P3', arrivalTime: 2, burstTime: 6, priority: 1, color: PRESET_COLORS[2] },
      { id: 'p4', name: 'P4', arrivalTime: 3, burstTime: 2, priority: 3, color: PRESET_COLORS[3] },
    ]);
    setAlgorithm('PRIOp');
    setIsOverloadEnabled(true);
    setOverloadTime(1);
    ...
    <div className="app-container">
      <header className="app-header">
        <div className="logo-container">
          ...
          <div>
            ...
            <p className="subtitle">Visualizador interativo de algoritmos de sistemas operacionais em tempo real</p>
          </div>
        </div>
      ...
      <main className="dashboard-grid">
        ...
        <section className="column config-panel">
          <div className="card glass">
            <h2 className="card-title">
              <svg className="icon-title" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="form-group">
              <label htmlFor="algorithm-select">Algoritmo de Escalonamento</label>
              <select
                id="algorithm-select"
                value={algorithm}
                onChange={(e) => {
            ...
            {algorithm === 'RR' && (
              <div className="form-group slide-in">
                <div className="label-with-val">
                  <label htmlFor="quantum-input">Quantum de Tempo (Execução contínua)</label>
            ...
            <div className="form-group">
              <div className="checkbox-container">
                ...
                />
                <label htmlFor="overload-toggle">Ativar Sobrecarga (Context Switch)</label>
              </div>
            </div>
            ...
            <div className="preset-quick-actions">
              ...
              <div className="button-group-wrap">
                <button onClick={handleLoadClassicPreset} className="btn btn-secondary btn-xs">
                  Round Robin Clássico
                </button>
                <button onClick={handleLoadConvoyPreset} className="btn btn-secondary btn-xs">
                  Efeito Comboio (FIFO)
                </button>
                <button onClick={handleLoadPriorityPreset} className="btn btn-secondary btn-xs">
          ...
          <div className="card glass">
            <div className="card-header-actions">
              ...
              <div className="header-actions">
                <button onClick={handleRandomize} title="Gerar Aleatório" className="btn btn-icon-only">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16">
                    <path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l5.67-5.67" />
                  </svg>
                </button>
                <button onClick={handleAddProcess} title="Adicionar Processo" className="btn btn-icon-only success">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16">
              ...
              <table className="process-table">
                <thead>
                  <tr>
                    ...
                    <th>Chegada</th>
                    <th>Execução</th>
                    {(algorithm === 'PRIOnp' || algorithm === 'PRIOp') && <th>Prioridade</th>}
                    <th>Cor</th>
                    <th></th>
                  </tr>
                </thead>
                    ...
                    <tr key={p.id} className="process-row">
                      ...
                      <td>
                        <input
                          ...
                          max="50"
                          value={p.arrivalTime}
                          onChange={(e) => handleUpdateProcess(p.id, 'arrivalTime', e.target.value)}
                      ...
                      <td>
                        <input
                          ...
                          max="30"
                          value={p.burstTime}
                          onChange={(e) => handleUpdateProcess(p.id, 'burstTime', e.target.value)}
                          className="table-input"
                        />
                      </td>
                      {(algorithm === 'PRIOnp' || algorithm === 'PRIOp') && (
                        <td>
                          <input
                            ...
                            max="10"
                            value={p.priority}
                            onChange={(e) => handleUpdateProcess(p.id, 'priority', e.target.value)}
                            className="table-input"
                          />
                        </td>
                      )}
                      <td>
                        <div className="color-picker-wrapper">
                          <input
                            type="color"
                            value={p.color}
                            onChange={(e) => handleUpdateProcess(p.id, 'color', e.target.value)}
        ...
        <section className="column main-visualization">
          {isLoading && (
            <div className="card glass loading-card" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '16px', padding: '16px' }}>
              <div className="spinner" />
              <div>
                <h3 style={{ fontSize: '15px' }}>Carregando simulação...</h3>
                <p style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>Enviando parâmetros para a biblioteca externa.</p>
              </div>
            </div>
          )}

          {error && (
            <div className="card glass error-card border-danger" style={{ borderLeft: '4px solid var(--accent-danger)', gap: '10px' }}>
              <div className="flex-align-center" style={{ gap: '10px' }}>
                <svg className="icon-warning animation-bounce" style={{ color: 'var(--accent-danger)' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="24" height="24">
                  <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
                  <line x1="12" y1="9" x2="12" y2="13" />
                  <line x1="12" y1="17" x2="12.01" y2="17" />
                </svg>
                <div>
                  <h3 style={{ color: 'var(--text-primary)', fontSize: '15px' }}>Falha na Conexão com Biblioteca Externa</h3>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '2px' }}>
                    Erro ao contatar o endpoint: <code className="font-mono" style={{ background: 'rgba(0,0,0,0.3)', padding: '2px 6px', borderRadius: '4px', fontSize: '12px' }}>{apiUrl}</code>
                  </p>
                </div>
              </div>
              <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                Certifique-se de que a biblioteca/API externa está ativa, aceitando requisições do tipo <code>POST</code> e que retorna a linha do tempo de execução.
              </p>
            </div>
          )}
          ...
          <div className="card glass playback-card">
            <div className="playback-layout">
              ...
              <div className="playback-controls">
                ...
                <button
                  onClick={() => setCurrentTime((prev) => Math.min(timeline.length - 1, prev + 1))}
            ...
            <div className="hardware-card glass cpu-socket-card">
              ...
              <div className={`cpu-socket ${currentStep.cpuState}`}>
                {currentStep.cpuState === 'running' && runningProcess ? (
                  <div className="cpu-running-state" style={{ '--proc-color': runningProcess.color } as React.CSSProperties}>
                    <div className="process-bubble pulse" style={{ backgroundColor: runningProcess.color }}>
                      {runningProcess.name}
                    </div>
                    <span className="process-status-text">Executando</span>
                    <span className="burst-info">Restante: {currentStep.processStates[runningProcess.id]?.remainingTime}s</span>
                  </div>
                ) : currentStep.cpuState === 'overload' ? (
                  <div className="cpu-overload-state striped-overload">
                    <svg className="icon-warning animation-bounce" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="28" height="28">
                      <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
                      <line x1="12" y1="9" x2="12" y2="13" />
                      <line x1="12" y1="17" x2="12.01" y2="17" />
                    </svg>
                    <span className="overload-text font-bold">SOBRECARGA</span>
                    <span className="overload-sub">Salvar/Restaurar Contexto</span>
                  </div>
                ) : (
                    ...
                    {currentStep.readyQueue.map((id, index) => {
                      ...
                      return (
                        <div key={proc.id} className="queue-item-wrapper">
                          <div className="queue-badge" style={{ backgroundColor: proc.color }}>
                            {proc.name}
                          </div>
                          {index < currentStep.readyQueue.length - 1 && (
                            <svg className="queue-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16">
                              <line x1="5" y1="12" x2="19" y2="12"></line>
          ...
          <div className="card glass gantt-card">
              ...
              <div className="gantt-chart">
                ...
                <div className="gantt-row time-header-row">
                  ...
                  <div className="gantt-row-cells">
                    {timeline.map((_, idx) => (
                      <div key={idx} className={`gantt-time-cell ${currentTime === idx ? 'active' : ''}`}>
                        {idx}
                      </div>
                    ))}
                    <div className="gantt-time-cell end-cap">{timeline.length}</div>
                  </div>
                </div>
                ...
                <div className="gantt-row cpu-row-gantt">
                  ...
                  <div className="gantt-row-cells">
                    {timeline.map((step, idx) => {
                      const proc = step.runningProcessId ? processes.find((p) => p.id === step.runningProcessId) : null;
                      ...
                      return (
                        <div
                          ...
                          style={{
                            backgroundColor: step.cpuState === 'running' && proc ? proc.color : undefined,
                          }}
                          onClick={() => {
                            setCurrentTime(idx);
                            setIsPlaying(false);
                        ...
                        >
                          {step.cpuState === 'running' && proc && <span className="cell-label">{proc.name}</span>}
                          {step.cpuState === 'overload' && <span className="cell-label xs-text">C</span>}
                        </div>
                      );
                    })}
                  </div>
                ...
                {processes.map((proc) => (
                  <div key={proc.id} className="gantt-row process-row-gantt">
                    <div className="gantt-row-label" style={{ borderLeft: `4px solid ${proc.color}` }}>
                      {proc.name}
                    </div>
                    <div className="gantt-row-cells">
                      {timeline.map((step, idx) => {
                        ...
                        if (!procState || procState.state === 'not_arrived') {
                          return <div key={idx} className="gantt-cell blank-cell" />;
                        }
                        ...
                        return (
                          <div
                            ...
                            className={cellClass}
                            style={{
                              backgroundColor: procState.state === 'running' ? proc.color : undefined,
                              borderColor: procState.state === 'ready' ? proc.color : undefined,
                            }}
                            onClick={() => {
                              setCurrentTime(idx);
                              setIsPlaying(false);
                          ...
                          >
                            {procState.state === 'running' && <span className="pulse-indicator" />}
                            {procState.state === 'ready' && <span className="ready-dot" style={{ backgroundColor: proc.color }} />}
                          </div>
                        );
                      })}
                    </div>
            ...
            <div className="gantt-legend">
              <div className="legend-item"><span className="legend-box running-legend" /> Executando</div>
              <div className="legend-item"><span className="legend-box ready-legend" /> Pronto (Fila)</div>
              <div className="legend-item"><span className="legend-box overload-legend striped-overload" /> Sobrecarga</div>
              <div className="legend-item"><span className="legend-box idle-legend" /> CPU Ociosa / Vazio</div>
            </div>
          </div>
          ...
          <div className="metrics-row">
            <div className="metric-card glass">
              ...
              <p className="metric-desc">Tempo médio que os processos ficaram na fila de prontos.</p>
            </div>
            
            <div className="metric-card glass">
              <span className="metric-label">Tempo Médio de Retorno</span>
              <span className="metric-value">{avgTurnaround}s</span>
              <p className="metric-desc">Tempo total da chegada até o término (Turnaround).</p>
            </div>

            <div className="metric-card glass">
              <span className="metric-label">Tempo Médio de Resposta</span>
              <span className="metric-value">{avgResponse}s</span>
              <p className="metric-desc">Tempo médio da chegada até começar a rodar na CPU.</p>
            </div>
          </div>
                      ...
                      return (
                        <tr key={p.id}>
                          <td className="font-bold flex-align-center">
                            <span className="badge-color-dot" style={{ backgroundColor: p.color }} />
                            {p.name}
                          </td>
      ...
      <footer className="app-footer">
        <p>Desenvolvido como simulador educacional de Sistemas Operacionais. &copy; {new Date().getFullYear()}</p>
      </footer>
    </div>
  );
}
